...
while ($gamestate -eq 1) {
    ...
    if ($p1input -eq "UCO" -or $p1input -eq "OCU" -or $p1input -eq "OU" -or $p1input -eq "OSU" -or $p1input -eq "Q") {
        ...
        if ($p1input -eq "OSU") {
            ...
            $cpurand = Get-Random -Minimum 0 -Maximum 4; #random number that isnt a tie
            $cpuchoice = $rps[$cpurand]
            if ($cpuchoice.paper -eq 1) {
                #if cpu loses to paper
                $p1points += 1;
                Write-Host "YOU WON YOU HAVE: $p1points" -ForegroundColor Green
                continue;
            }
            elseif ($cpuchoice.paper -eq 0) {
                #if cpu wins to paper
                $cpupoints += 1;
                Write-Host "CPU WON THIS ROUND, CPU POINTS: $cpupoints" -ForegroundColor Red
                continue;
            }
        ...
        if ($p1input -eq "UCO") {
            #p1 chooses UCO, AUTO WIN IF NOT TIE
            $cpurand = Get-Random -Minimum 0 -Maximum 4; #random number that isnt a tie
            $cpuchoice = $rps[$cpurand]
            if ($cpuchoice.scissors -ne 3) {
                #if cpu loses to UCO
                $p1points += 1;
                Write-Host "YOU WON YOU HAVE: $p1points" -ForegroundColor Green
                continue;
        ...
        if ($p1input -eq "OCU") {
            ...
            $cpurand = Get-Random -Minimum 0 -Maximum 4; #random number that isnt a tie
            $cpuchoice = $rps[$cpurand]
            if ($cpuchoice.scissors -eq 1) {
                #if cpu loses to scissors
                $p1points += 1;
                Write-Host "YOU WON YOU HAVE: $p1points" -ForegroundColor Green
                continue;
            }
            elseif ($cpuchoice.scissors -eq 0) {
                #if cpu wins to scisoors
                $cpupoints += 1;
                Write-Host "CPU WON THIS ROUND, CPU POINTS: $cpupoints" -ForegroundColor Red
                continue;
            }
        ...
        Write-Host "invalid p1input, please try again" -ForegroundColor Magenta
        continue
    }
}